...
namespace :gtfs do
  namespace :import do
    ...
    #task :all => [:trips, :routes, :stop_times, :stops, ]
    
    desc "Import Trips"
    task :trips => :environment do
      Trip.delete_all if Trip.all.any?
      print "Importing Trip records..."
      rows = CSV.read("#{Rails.root}/data/trips.txt")
    ...
    task :routes => :environment do
      ...
      rows = CSV.read("#{Rails.root}/data/routes.txt")
      rows[1..rows.length].each do |column|
        route = Route.new
    ...
    task :stop_times => :environment do
      ...
      rows = CSV.read("#{Rails.root}/data/stop_times.txt")
      rows[1..rows.length].each do |column|
    ...
    task :stops => :environment do
      ...
      rows = CSV.read("#{Rails.root}/data/stops.txt")
      rows[1..rows.length].each do |column|
        stop = Stop.new
    ...
    task :calendars => :environment do
      ...
      calendars = CSV.read("#{Rails.root}/data/calendar.txt")
